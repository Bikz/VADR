...
export async function enrichCallData(call: Call): Promise<EnrichmentResult | null> {
  ...
  try {
    ...
    // Merge with existing extractedData, normalizing skus to array
    const merged: EnrichmentResult = {
      ...existingData,
      ...enrichment,
      // Normalize skus to array
      skus: enrichment.skus
        ? (Array.isArray(enrichment.skus) ? enrichment.skus : [enrichment.skus])
        : existingData?.skus,
    };
